...
public class FetchServiceImp implements FetchService {
	...
	@Override
	public OperationResponse FetchCategories(String placeName) {

		List<Place> places = gClient().getPlacesByQuery(placeName, GooglePlaces.MAXIMUM_RESULTS);
		GeoApiContext context = new GeoApiContext.Builder().apiKey("AIzaSyBFNRmLV8bEPw15tE1fN79y-rlhTmw5Lso").build();
		...
		try {
			...
			if (place == null) {
				...
				if (details.photos != null && details.photos.length > 0) {
					...
					String url = "https://maps.googleapis.com/maps/api/place/photo?maxwidth=1200&photoreference=" + ref
							+ "&key=";
